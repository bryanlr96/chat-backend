...
class SocketController {
  ...
  static async getAllRequests(userId, socket) {
    try {
      ...
      socket.emit('all_requests', requests);
    } catch (error) {
      console.error('Error al obtener las solicitudes:', error);
      socket.emit('error', 'Hubo un problema al obtener las solicitudes');
      socket.emit('all_requests', []);
    }
  }


  // Metodo para rechazar una solicitud
  static async rejectRequest(userId, socket, request_ID) {
    try {
      const result = await SocketModel.rejectRequest(request_ID)
  ...
  static async acceptRequest(receptor_ID, socket, request_ID, connectedUsers, io) {
    try {
      // Recuperar los id de los usuarios implicados y añadimos el contacto
      const emisor_ID = await SocketModel.getEmisor(request_ID);
      const addResult = await SocketModel.addContact(receptor_ID, emisor_ID);

      if (!addResult) return; // Si no se añade correctamente

      // Eliminamos la solicitud
      await SocketModel.rejectRequest(request_ID);

      // Mostramos la lista de solicitudes actualizadas
      const updatedRequests = await SocketModel.getAllRequests(receptor_ID);
      socket.emit('all_requests', updatedRequests);

      // Mostramos los contactos actualizados a ambos usuarios
      await Promise.all([
        emitUpdatedContacts(receptor_ID, connectedUsers, io, SocketModel),
        emitUpdatedContacts(emisor_ID, connectedUsers, io, SocketModel)
      ]);
  ...
  static async sendRequest(emisor_ID, socket, email, connectedUsers, io) {
    try {
      ...
      if (createRequest) {
        ...
        if (receptorSocketId) {
          io.to(receptorSocketId).emit('all_requests', requestsReceptor);
        }
      } else {
  ...
  static async addMessage(io, connectedUsers, message, userId, socket, contactId) {
    try {
      const addedMessage = await SocketModel.addMessage(contactId, userId, message);
      ...
      for (const userID of [user1_ID, user2_ID]) {
        ...
        if (socketId) {
          io.to(socketId).emit('allMessages', messages);
        }
      }
    } catch (error) {
